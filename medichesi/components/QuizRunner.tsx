      ...
      <div style={{ ['--dc' as string]: color }}>
        ...
        {answered && (
          <div className="flex flex-col gap-[10px] mt-1 mb-[14px]">
            ...
            {/* Why each WRONG option is wrong - shown ALWAYS, whether user got it right or not */}
            {current.scenario.opts.map((_, i) => {
              if (i === current.scenario.ans) return null
              const rationale = current.scenario.exp.wrong[i]
              if (!rationale) return null
              const userPicked = answered.selected === i
              return (
                <div
                  key={i}
                ...
                >
                  ...
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="font-mono text-[10px] uppercase tracking-[0.14em] text-danger">
                      Why {LETTERS[i]} is wrong{userPicked ? ' (your pick)' : ''}
                    </span>
                  </div>
                  <p className="text-[14.5px] leading-relaxed">{rationale}</p>
                </div>
              )
            })}
